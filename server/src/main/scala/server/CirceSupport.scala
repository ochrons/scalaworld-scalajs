...
trait CirceSupport {
  ...
  val jsonPrinter = Printer(preserveOrder = true, dropNullValues = true, "")
  ...
  implicit def circeUnmarshaller[A](
    ...
    jsonStringUnmarshaller.map(data => jawn.decode(data).right.get)
  ...
  implicit def circeToEntityMarshaller[A](
    ...
    printer: Json => String = jsonPrinter.pretty
